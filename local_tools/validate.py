"""VALIDATE.PY VERSION 3.3
Validates user input and handles errors
Includes:
- log_error(reason): print an error statement to the console with the provided reason.
- ask(prompt:str, conditions:list, errors:list, clear_console:bool=False, variables:dict={}, disallow_null:bool=True): asks the user a question 'prompt', checking it against every element in the list 'conditions', printing the corresponding element in the list 'errors' on a condition failure and asking the user to input an answer again; variables used in 'conditions' can be given to the ask() function via the dictionary 'variables' and used in 'conditions' using the syntax var['variable_name_here']; if clear_console is True, the console will be cleared upon failure and before reprinting the prompt
"""
...
def ask(prompt:str, conditions:list, errors:list, clear_console:bool=False, variables:dict={}, disallow_null:bool=True):
  ...
  - clear console must be a boolean
